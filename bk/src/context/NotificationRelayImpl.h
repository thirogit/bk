#ifndef __NOTIFICATIONRELAYIMPL_H__
#define __NOTIFICATIONRELAYIMPL_H__

#include <service\notification\NotificationRelay.h>
#include "HerdObserver.h"
#include <logic/herdregistry/CowPresenceKey.h>

class SeasonSessionImpl;

class NotificationRelayImpl : public NotificationRelay
{
public:
	NotificationRelayImpl(SeasonSessionImpl* pOwningSession);
...
private:
	void VisitTrail(uint32_t cowId,SeasonSessionImpl* pSession,std::function<void (HerdObserver*,CowPresenceKey&)>& visitor);
private:
	SeasonSessionImpl* m_pOwningSession;
};
