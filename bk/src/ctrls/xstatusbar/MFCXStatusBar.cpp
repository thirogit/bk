...
inline CMFCXStatusBarPaneInfo* CMFCXStatusBar::_GetPanePtr(int nIndex) const
{
	if (nIndex == 255 && m_nCount < 255)
	{
		...
		{
			CMFCXStatusBarPaneInfo* pSBP = _GetPanePtr(i);
			ENSURE(pSBP != NULL);
	...
	return((CMFCXStatusBarPaneInfo*)m_pData) + nIndex;
}
...
void CMFCXStatusBar::OnSettingChange(UINT /*uFlags*/, LPCTSTR /* lpszSection */)
{
	RecalcLayout();
}
...
BOOL CMFCXStatusBar::PreCreateWindow(CREATESTRUCT& cs)
{
...
BOOL CMFCXStatusBar::Create(CWnd* pParentWnd, DWORD dwStyle, UINT nID)
{
	return CreateEx(pParentWnd, 0, dwStyle, nID);
}

BOOL CMFCXStatusBar::CreateEx(CWnd* pParentWnd, DWORD /*dwCtrlStyle*/, DWORD dwStyle, UINT nID)
{
...
BOOL CMFCXStatusBar::SetIndicators(const UINT* lpIDArray, int nIDCount)
{
	ASSERT_VALID(this);
	...
	if (!AllocElements(nIDCount, sizeof(CMFCXStatusBarPaneInfo)))
		return FALSE;
...
/////////////////////////////////////////////////////////////////////////////
// CMFCXStatusBar attribute access

int CMFCXStatusBar::CommandToIndex(UINT nIDFind) const
{
	ASSERT_VALID(this);
	...
	CMFCXStatusBarPaneInfo* pSBP = _GetPanePtr(0);
	if (pSBP == NULL)
	{
		ASSERT(FALSE);
		return -1;
...
UINT CMFCXStatusBar::GetItemID(int nIndex) const
{
	ASSERT_VALID(this);

	CMFCXStatusBarPaneInfo* pSBP = _GetPanePtr(nIndex);
	if (pSBP == NULL)
	{
		ASSERT(FALSE);
		return 0;
...
void CMFCXStatusBar::GetItemRect(int nIndex, LPRECT lpRect) const
{
	ASSERT_VALID(this);
...
UINT CMFCXStatusBar::GetPaneStyle(int nIndex) const
{
	CMFCXStatusBarPaneInfo* pSBP = _GetPanePtr(nIndex);
	if (pSBP == NULL)
	{
...
void CMFCXStatusBar::SetPaneStyle(int nIndex, UINT nStyle)
{
	CMFCXStatusBarPaneInfo* pSBP = _GetPanePtr(nIndex);
	if (pSBP == NULL)
	{
...
int CMFCXStatusBar::GetPaneWidth(int nIndex) const
{
	CMFCXStatusBarPaneInfo* pSBP = _GetPanePtr(nIndex);
	if (pSBP == NULL)
	{
...
void CMFCXStatusBar::SetPaneWidth(int nIndex, int cx)
{
	CMFCXStatusBarPaneInfo* pSBP = _GetPanePtr(nIndex);
	if (pSBP == NULL)
	{
...
void CMFCXStatusBar::GetPaneInfo(int nIndex, UINT& nID, UINT& nStyle, int& cxWidth) const
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::SetPaneInfo(int nIndex, UINT nID, UINT nStyle, int cxWidth)
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::GetPaneText(int nIndex, CString& s) const
{
	ASSERT_VALID(this);
...
CString CMFCXStatusBar::GetPaneText(int nIndex) const
{
	ASSERT_VALID(this);
...
BOOL CMFCXStatusBar::SetPaneText(int nIndex, LPCTSTR lpszNewText, BOOL bUpdate)
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::SetPaneIcon(int nIndex, HICON hIcon, BOOL bUpdate)
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::SetPaneIcon(int nIndex, HBITMAP hBmp, COLORREF clrTransparent, BOOL bUpdate)
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::SetPaneAnimation(int nIndex, HIMAGELIST hImageList, UINT nFrameRate, BOOL bUpdate)
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::EnablePaneProgressBar(int nIndex, long nTotal, BOOL bDisplayText, COLORREF clrBar, COLORREF clrBarDest, COLORREF clrProgressText)
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::SetPaneProgress(int nIndex, long nCurr, BOOL bUpdate)
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::SetPaneTextColor(int nIndex, COLORREF clrText, BOOL bUpdate)
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::SetPaneBackgroundColor(int nIndex, COLORREF clrBackground, BOOL bUpdate)
{
	ASSERT_VALID(this);
...
CString CMFCXStatusBar::GetTipText(int nIndex) const
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::SetTipText(int nIndex, LPCTSTR pszTipText)
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::InvalidatePaneContent(int nIndex)
{
	if (GetSafeHwnd() == NULL)
	{
		return;
...
void CMFCXStatusBar::EnablePaneDoubleClick(BOOL bEnable)
{
	m_bPaneDoubleClick = bEnable;
}

/////////////////////////////////////////////////////////////////////////////
// CMFCXStatusBar implementation

CSize CMFCXStatusBar::CalcFixedLayout(BOOL, BOOL bHorz)
{
	ASSERT_VALID(this);
	...
	int cyIconMax = 0;
	CMFCXStatusBarPaneInfo* pSBP = (CMFCXStatusBarPaneInfo*)m_pData;
	for (int i = 0; i < m_nCount; i++, pSBP++)
	{
...
void CMFCXStatusBar::DoPaint(CDC* pDCPaint)
{
	ASSERT_VALID(this);
...
/////////////////////////////////////////////////////////////////////////////
// CMFCXStatusBar message handlers

BEGIN_MESSAGE_MAP(CMFCXStatusBar, CPane)
	ON_WM_NCHITTEST()
	ON_WM_SYSCOMMAND()
	ON_WM_SIZE()
	ON_WM_SETTINGCHANGE()
...
int CMFCXStatusBar::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CMFCXStatusBar::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	if (m_bPaneDoubleClick)
	{
		CMFCXStatusBarPaneInfo* pSBP = HitTest(point);
		if (pSBP != NULL)
		{
...
void CMFCXStatusBar::OnTimer(UINT_PTR nIDEvent)
{
...
LRESULT CMFCXStatusBar::OnNcHitTest(CPoint point)
{
...
void CMFCXStatusBar::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CMFCXStatusBar::OnSize(UINT nType, int cx, int cy)
{
...
void CMFCXStatusBar::OnSetFont(CFont* pFont, BOOL bRedraw)
{
...
HFONT CMFCXStatusBar::OnGetFont()
{
	return GetCurrentFont();
}

int CMFCXStatusBar::OnSetText(LPCTSTR lpszText)
{
...
int CMFCXStatusBar::OnGetText(int nMaxChars, LPTSTR lpszText)
{
...
UINT CMFCXStatusBar::OnGetTextLength()
{
	int nLen = 0;
	int nIndex = CommandToIndex(0); // use pane with ID zero
	if (nIndex >= 0)
...
void CMFCXStatusBar::OnDrawPane(CDC* pDC, CMFCXStatusBarPaneInfo* pPane)
{
	ASSERT_VALID(pDC);
	ENSURE(pPane != NULL);
...
void CMFCXStatusBar::RecalcLayout()
{
	ASSERT_VALID(this);
	...
	for (i = 0, pSBP = (CMFCXStatusBarPaneInfo*)m_pData; i < m_nCount; i++, pSBP++)
	{
		ASSERT(pSBP->cxText >= 0);
...
/////////////////////////////////////////////////////////////////////////////
// CMFCXStatusBar idle update through CMFCXStatusBarCmdUI class

class CMFCXStatusBarCmdUI : public CCmdUI      // class private to this file!
{
...
void CMFCXStatusBarCmdUI::Enable(BOOL bOn)
{
	m_bEnableChanged = TRUE;
	CMFCXStatusBar* pStatusBar = (CMFCXStatusBar*)m_pOther;
	ENSURE(pStatusBar != NULL);
	ASSERT_KINDOF(CMFCXStatusBar, pStatusBar);
...
void CMFCXStatusBarCmdUI::SetCheck(int nCheck) // "checking" will pop out the text
{
	CMFCXStatusBar* pStatusBar = (CMFCXStatusBar*)m_pOther;
	ENSURE(pStatusBar != NULL);
	ASSERT_KINDOF(CMFCXStatusBar, pStatusBar);
...
void CMFCXStatusBarCmdUI::SetText(LPCTSTR lpszText)
{
	ENSURE(m_pOther != NULL);
	ASSERT_KINDOF(CMFCXStatusBar, m_pOther);
	ASSERT(m_nIndex < m_nIndexMax);

	((CMFCXStatusBar*)m_pOther)->SetPaneText(m_nIndex, lpszText);
}

void CMFCXStatusBar::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
	CMFCXStatusBarCmdUI state;
	state.m_pOther = this;
...
INT_PTR CMFCXStatusBar::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	ASSERT_VALID(this);
...
CMFCXStatusBarPaneInfo* CMFCXStatusBar::HitTest(CPoint pt) const
{
	ASSERT_VALID(this);
...
long CMFCXStatusBar::GetPaneProgress(int nIndex) const
{
	ASSERT_VALID(this);
...
HFONT CMFCXStatusBar::GetCurrentFont() const
{
	return m_hFont == NULL ? (HFONT) GetGlobalData()->fontRegular.GetSafeHandle() : m_hFont;
}

void CMFCXStatusBar::OnStyleChanged(int nStyleType, LPSTYLESTRUCT lpStyleStruct)
{
...
/////////////////////////////////////////////////////////////////////////////
// CMFCXStatusBar diagnostics

#ifdef _DEBUG
void CMFCXStatusBar::AssertValid() const
{
	CPane::AssertValid();
}

void CMFCXStatusBar::Dump(CDumpContext& dc) const
{
	CPane::Dump(dc);
...
IMPLEMENT_DYNAMIC(CMFCXStatusBar, CPane)

BOOL CMFCXStatusBar::GetExtendedArea(CRect& rect) const
{
	ASSERT_VALID(this);
...
void CMFCXStatusBar::OnShowWindow(BOOL bShow, UINT nStatus)
{
