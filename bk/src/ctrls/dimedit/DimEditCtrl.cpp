...
BEGIN_MESSAGE_MAP(CDimEditCtrl, CStdEdit)
...
{
	CStdEdit::PreSubclassWindow();								// Do Default...
...
void	CDimEditCtrl::SetDimText( LPCTSTR cpDimText )
{
	...
	{
		_tcsncpy_s( m_caDimText, cpDimText, DIM_TEXT_LEN );	// Copy Over The Text
...
LRESULT CDimEditCtrl::OnSetText(WPARAM wParam, LPARAM lParam)
{
...
{
	BOOL	bStatus = CStdEdit::OnEraseBkgnd(pDC);
...
{
	TRACE( _T( "Click...\n" ) );
	CStdEdit::OnLButtonDown(nFlags, point);
}

void CDimEditCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	TRACE( _T( "DClick...\n" ) );
	CStdEdit::OnLButtonDblClk(nFlags, point);
}
*/
...
{
	CStdEdit::OnSettingChange(uFlags, lpszSection);
