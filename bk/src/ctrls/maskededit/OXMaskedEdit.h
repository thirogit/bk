...
class OX_CLASS_DECL COXMaskedEdit : public CStdEdit
{
...
public:
	...
	// --- Effect : 
	std::wstring GetInputData() const ;
	...
	// --- In     :
	// --- Out    : 
	// --- Returns: TRUE if in all prompt symbols were typed valid characters 
	// --- Effect : 

	BOOL IsFullPrompt() const;
	...
	// --- In     : TRUE , control won't lost focus if all dat isn't typed in
	// --- Out    :
	// --- Returns: 
	// --- Effect : 

	void SetNeedAllData(BOOL bAllData);
...
protected:
	...
	void    Update                  (int nSelectionStart=0,bool bModify = FALSE) ;  
