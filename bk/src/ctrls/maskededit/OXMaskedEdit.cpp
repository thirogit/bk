...
std::wstring COXMaskedEdit::GetInputData() const
{
	std::wstring csInputData;
...
BOOL COXMaskedEdit::SetInputData(LPCTSTR pszInputData, int nBeginPos/*=0*/, BOOL bAllowPrompt/*=TRUE*/)
{
	std::wstring csFullInput;
	// Start with existing data and append the new data. 
	csFullInput=GetInputData();
	csFullInput=TextUtils::Left(csFullInput,nBeginPos);
	if(bAllowPrompt)
	{
	...
	LPCTSTR pszReplaceData=csFullInput.c_str();
	CMaskData* pobjData=NULL;
	for(POSITION pos=m_listData.GetHeadPosition(); pos;)
	{
		pobjData=m_listData.GetNext(pos);
...
BOOL COXMaskedEdit::IsFullPrompt() const
{
...
BOOL COXMaskedEdit::IsInputEmpty() const
{
	if(m_listData.GetCount()==0)
	{
		CString csInputData;
...
void COXMaskedEdit::SetNeedAllData(BOOL bAllData)
{

	m_bNeedAllData = bAllData;
...
int COXMaskedEdit::InsertAt(int nSelectionStart, TCHAR chNewChar)
{
	...
	std::wstring csPreviousInput = GetInputData();
	...
	if(!SetInputData(csInputData.c_str()))
	{
		// If not successful, then restore the previous input and return -1. 
		SetInputData(csPreviousInput.c_str());
		return -1;
	}
...
void COXMaskedEdit::Update(int nSelectionStart/*=0*/,bool bModify)
{
	...
	if(::IsWindow(m_hWnd))
	{
		...
		if(bModify /*&& !GetModify()*/) SetModify();
	}
}
...
void COXMaskedEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	BOOL bEnabled = IsWindowEnabled() && !(GetStyle() & ES_READONLY);
...
BOOL COXMaskedEdit::OnValidate() 
{
	...
	if(m_bNeedAllData && !IsInputEmpty() && !IsFullPrompt())
		return FALSE;
