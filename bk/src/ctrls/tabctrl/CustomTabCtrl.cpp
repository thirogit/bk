	...
	default:
		{
			...
			else
			{
				...
				if(bNotify && m_nItemSelected >= 0)
					NotifyParent(CTCN_TABUNSELECTED,m_nItemSelected,point);

				SetCurSel(nHitTest,TRUE,nFlags&MK_CONTROL);
				if(bNotify)
					NotifyParent(CTCN_TABSELECTED,m_nItemSelected,point);
			}
			for(int i=0; i<m_aItems.GetSize();i++)
			{
...
BOOL CCustomTabCtrl::NotifyParent(UINT code, int nItem, CPoint pt)
{
	if(nItem>=0)
	{
		...
		nmh.nItem = nItem;
		nmh.ptHitTest = pt;	
		_tcscpy_s(nmh.pszText,m_aItems[nItem]->m_sText);
...
int CCustomTabCtrl::SetCurSel(int nItem,BOOL bNotify/* = FALSE*/)
{
	if(m_nItemSelected != nItem)
	{
		CPoint point;
		NotifyParent(CTCN_TABUNSELECTED,m_nItemSelected,point);
		int ret = SetCurSel(nItem,FALSE,FALSE);
		NotifyParent(CTCN_TABSELECTED,m_nItemSelected,point);
		return ret;
	}
	return CTCERR_NOERROR;
...
void CCustomTabCtrl::RecalcEditResized(int nOffset, int nItem)
{
	...
	do
	{
		...
		int h = pDC->DrawText(m_aItems[nItem]->m_sText+L"X", r, DT_CALCRECT);
