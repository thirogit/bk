...
void CXTabCtrl::AddTab(CWnd* pWnd, LPCTSTR lpszCaption, int iImage)
{
	...
	LPTSTR pTabCaption = const_cast<LPTSTR>(lpszCaption);


	TCITEM item;
	item.mask = TCIF_TEXT|TCIF_PARAM|TCIF_IMAGE;
	item.lParam = (LPARAM) pWnd;
	item.pszText = pTabCaption;
	item.iImage = iImage;
	...
	CRect rect; 
	GetClientRect(&rect);
	rect.DeflateRect(m_ptTabs.x, m_ptTabs.y);

	pWnd->SetWindowPos(NULL, m_ptTabs.x, m_ptTabs.y , rect.Width(), rect.Height(),
...
void CXTabCtrl::OnSize(UINT nType, int cx, int cy) 
{
	...
	for (int i=0; i < GetItemCount(); i++)
	{
		...
		pWnd->SetWindowPos(NULL,0, 0 , cx-8, cy-38,SWP_FRAMECHANGED | SWP_NOZORDER | SWP_NOMOVE);	
	}	
...
int CXTabCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
