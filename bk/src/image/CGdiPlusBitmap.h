...
public:
	...
	CGdiPlusBitmap(LPCWSTR pFile)				{ m_pBitmap = NULL; LoadFile(pFile); }
	virtual ~CGdiPlusBitmap()					{ Empty(); }

	void Empty()								{ delete m_pBitmap; m_pBitmap = NULL; }

	bool LoadFile(LPCWSTR pFile)
	{
		Empty();
...
public:
	...
	CGdiPlusBitmapResource(LPCTSTR pName, LPCTSTR pType = RT_RCDATA, HMODULE hInst = NULL)
												{ m_hBuffer = NULL; LoadResource(pName, pType, hInst); }
	...
	bool LoadResource(LPCTSTR pName, LPCTSTR pType = RT_RCDATA, HMODULE hInst = NULL);
	bool Load(UINT id, LPCTSTR pType = RT_RCDATA, HMODULE hInst = NULL)
												{ return LoadResource(MAKEINTRESOURCE(id), pType, hInst); }
	bool Load(UINT id, UINT type, HMODULE hInst = NULL)
												{ return LoadResource(MAKEINTRESOURCE(id), MAKEINTRESOURCE(type), hInst); }
};
...
inline bool CGdiPlusBitmapResource::LoadResource(LPCTSTR pName, LPCTSTR pType, HMODULE hInst)
{
	Empty();
