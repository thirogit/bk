...
CRect CRulerWnd::GetBorderSize() const
{
	  ...
	  return CRect(clientRect.top-wndRect.top,
					clientRect.left-wndRect.left,
					wndRect.right-clientRect.right,
					wndRect.bottom-clientRect.bottom);
}
...
BOOL CRulerWnd::DrawHRuler(CDC *pDC)
{
  ...
  for( iSayac = 0 ; iSayac <= (int)m_nRulerMargin; iSayac++ ) {
  ...
  for( iSayac = 0 ; iSayac < m_Seperators.GetSize() ; iSayac++ ) {
